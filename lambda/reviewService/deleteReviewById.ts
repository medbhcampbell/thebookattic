...
export const handler = async (event: MyEvent): Promise<any> => {
    ...
    let q = `select bookid from reviews where id=$1::int`
    ...
    if(reviewsLeftQuery.rows.length > 0) {
        ...
        q = `update authors set avgrating=0 where id=(select authorid from books where id=$1::int);`;
        await client.query(q, [bookid]).catch((err: any) => console.error(err));

        q = `update books set rating=0 where id=$1::int;`;
        await client.query(q, [bookid]).catch((err: any) => console.error(err));
    }
